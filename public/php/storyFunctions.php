...
function loadTargetPage($page_id){
	...
	$realImageLink = $mysqlObject->queryDataBase('SELECT path FROM page_images WHERE id = "'.$page_information[0]['imageLink'].'"');
	
	$usedImageLink = '';
	
	if(isset($realImageLink[0]['path'])){
		$usedImageLink = $realImageLink[0]['path'];
	}
	
	array_push($returnArray, $page_information[0]['title']);
	array_push($returnArray, $page_information[0]['text']);
	array_push($returnArray, $usedImageLink);
